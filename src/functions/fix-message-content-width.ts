 ...
 * 修复消息内容宽度
 */
export function fixMessageContentWidth(messageUserList: IMessageUserList) {
  messageUserList.forEach((message) => {
    ...
    const stickers = messageElement.querySelectorAll(
      ".vac-message-container .vac-message-card div > div > span"
    ...
    if (stickers && stickers.length > 1) {
      // 如果这个 sticker 里面有多个 span，那么就需要考虑是不是有其他的 sticker
      stickers.forEach((sticker, index) => {
        ...
        const before =
          stickers[index - 1] &&
          stickers[index - 1].querySelector(".vac-message-content");
        const after =
          stickers[index + 1] &&
          stickers[index + 1].querySelector(".vac-message-content");
        if (before || after) {
